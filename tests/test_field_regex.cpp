...
void test_regex_group(const ti::path_t &field_dir, const std::string &field, const ti::path_t &query_path,
                const ti::path_t &regex_result_path)
{
    ...
    auto regex_vec = ti::load_regex_vec(query_path);
    std::ofstream ofs;
...
int main()
{
    test_regex_group(field_dir, field, query_path, regex_result_path);
    return 0;
}