...
namespace ti
{
    ...
    public:
        ...
        const doc_id_position_offset_vec_t retrieve(const token_t &token) const;
        const doc_id_set_t retrieve_union(const query_t &query) const;
        const doc_id_position_offset_vec_t retrieve_intersection(const query_t &query) const;
    private:
